...
This closes only current-contract freeze. It does not pass target parity, activate the privacy remediation, or authorize extraction. WF-PRE-06 subsequently approved the [ten-operation behavior matrix](workforce-behavior-matrix.md), WF-PRE-07 approved the [bounded-context seams](workforce-bounded-context-seams.md), WF-PRE-08 approved [exact table access](workforce-table-ownership-access.md), and WF-PRE-09 approved [public Application contracts](workforce-public-application-contracts.md); the next ordered prerequisite is WF-PRE-10.
