...
This closes only current-contract freeze. It does not define the target public Application facade, pass target parity, resolve the privacy mismatch, or authorize extraction. WF-PRE-06 subsequently approved the [ten-operation behavior matrix](workforce-behavior-matrix.md), WF-PRE-07 approved the [bounded-context seams](workforce-bounded-context-seams.md), and WF-PRE-08 approved [exact table access](workforce-table-ownership-access.md); the next ordered prerequisite is WF-PRE-09.
